...
export default class RideRepositoryDatabase implements RideRepository {
  ...
  async getById(rideId: string) {
    ...
    return new Ride(ride.ride_id, ride.passenger_id, ride.driver_id, ride.status, ride.date, parseFloat(ride.from_lat), parseFloat(ride.from_long), parseFloat(ride.to_lat), parseFloat(ride.to_long), parseFloat(ride.distance), parseFloat(ride.fare), lastPosition)
  }
  ...
  async list(): Promise<Ride[]> {
    ...
    for (const rideData of ridesData) {
      rides.push(new Ride(rideData.ride_id, rideData.passenger_id, rideData.driver_id, rideData.status, rideData.date, parseFloat(rideData.from_lat), parseFloat(rideData.from_long), parseFloat(rideData.to_lat), parseFloat(rideData.to_long), parseFloat(rideData.distance), parseFloat(rideData.fare), new Coord(parseFloat(rideData.last_lat), parseFloat(rideData.last_long))))
    }
    return rides
  }
